...
class LinkLabFlutterPlugin : FlutterPlugin, MethodCallHandler, ActivityAware, NewIntentListener {
    ...
    private var pendingDynamicIntent: Intent? = null
    private var isLinkLabInitialised: Boolean = false
    ...
    override fun onMethodCall(@NonNull call: MethodCall, @NonNull result: Result) {
        ...
        when (call.method) {
            "init" -> {
                ...
                val networkTimeout =
                    (configMap?.get("networkTimeout") as? Number)?.toDouble() ?: 30.0
                val networkRetryCount =
                    (configMap?.get("networkRetryCount") as? Number)?.toInt() ?: 3
                ...
                Log.d(
                    TAG,
                    "Initializing with custom domains: $domains, debug: $debugLoggingEnabled"
                )
                linkLab?.init(config)
                ...
                pendingDynamicIntent?.let {
                    Log.d(
                        TAG,
                        "Processing previously pending intent after initialisation: ${it.data}"
                    )
                    processIntent(it)
